...
def build_segment_from_sentences(sentences: List[str], start_idx: int, min_chars: int = 180, max_chars: int = 420) -> str:
    chunk = []
    total = 0
    ...
    for i in range(start_idx, len(sentences)):
        ...
        if total >= min_chars:
            # 조금 더 자연스러우면 한두 문장 더 붙이되 max_chars 안 넘기기
            continue

    return " ".join(chunk).strip()


def pick_short_segments(full_script: str, count: int = 3, min_chars: int = 180, max_chars: int = 420) -> List[str]:
    ...
    for sp in start_points:
        seg = build_segment_from_sentences(sentences, sp, min_chars=min_chars, max_chars=max_chars)
        if seg:
            segments.append(seg)
    ...
    while len(segments) < count and idx < len(sentences):
        seg = build_segment_from_sentences(sentences, idx, min_chars=min_chars, max_chars=max_chars)
...
def generate():
    try:
        ...
        shorts_segments = pick_short_segments(
            ...
            count=3,
            min_chars=180,   # 대략 30~60초 분량을 위한 보수적 범위
            max_chars=420
        )
...
def save_script():
    ...
    try:
        ...
        return jsonify({
            "status": "saved",
            "message": "대본이 저장되었습니다. 현재 버전에서는 scripts.json에 저장되며, 바로 재검색 대상에 포함됩니다."
        })
